...
def colorize(diff, css=True):
    ...
    return html_fmt.format(css=css, body="\n".join(_colorize(diff)))

def _colorize(diff):
    lines = diff.splitlines() if isinstance(diff, basestring) else diff
    lines.reverse()
...
if __name__ == "__main__":
    a = editor.get_text() #sys.argv[0]
    b = clipboard.get() #sys.argv[2]
